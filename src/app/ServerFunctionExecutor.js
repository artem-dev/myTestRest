...
//
//*******************************************************
let convertStringToBase64 = require('./base64').convertStringToBase64; 
let XMLHttpRequest = require('xmlhttprequest').XMLHttpRequest;
// let jsdom = require('jsdom').jsdom;
let navigator = require('navigator').navigator;
// let url = require('url');
// let os = require('os');
// let hostname = os.hostname();

$$USE_SESSION_COOKIE$$ = false;
$$SESSION_COOKIE_PREFIX$$ = 'JULIVI';
...
function getAuth(userName, password, alertOnFailure) {
    if (isReferenceAFunction(convertStringToBase64)) {
        let authStr = convertStringToBase64(userName + ':' + password);
...
function ServerFunctionExecutor(className, connectionInfo, owner) {
    ...
    this.getCacheURL = function (URLSuffix, includeSessionId) {
        ...
        let url = this.getURLPrefix(false) + this.cacheContext;
        let hasParams = false;
        if (URLSuffix != null) {
            URLSuffix += '';
    ...
    this.getURLPrefix = function (includeRest) {
        ...
        let portString = isValidPort(this.port) ? ':' + this.port : '';
        let dsAndRestSegments = '/' + this.dsContext;
        if (includeRest) {
            dsAndRestSegments += this.restContext;
        }
        let httpPrefix = this.isHttpS ? 'https://' : 'http://';
    ...
    this.getMethodURL = function (methodName, requestType, params, requestFilters) {
        ...
        //optionally using the 'pathPrefix' property which could be contributed through connectionInfo
        let portString = isValidPort(this.port) ? ':' + this.port : '';
        let dsAndRestSegments = '/' + this.dsContext + this.restContext;
   ...
   */
    ...
    this.closeSession = function () {
        ...
        let url = this.getURLPrefix() + 'CloseSession/';
        let result = this.executeMethodURL(url, null, 'GET', null, true, null);
        setSessionData(null, null);
        return result;
    };
}
exports.ServerFunctionExecutor = ServerFunctionExecutor;
/*
...
function getConnectionHost(connectionInfo) {
    ...
    let host = 'localhost';
    // let hostAndPort = os.hostname();
    // if (hostAndPort != 'localhost') {
    //     if (hostAndPort.indexOf(':') > -1) {
    //         let splits = hostAndPort.split(':', 2);
    //         host = splits[0];
    //     }
    //     else {
    //         host = hostAndPort;
...
function getDSContext(connectionInfo) {
    if (connectionInfo != null && connectionInfo.dscontext != null) {
        let result = trimString(connectionInfo.dscontext);
        if (result == '') {
            return '';
        }
        return result + '/';
...
function getRestContext(connectionInfo) {
    if (connectionInfo != null && connectionInfo.restcontext != null) {
        let result = trimString(connectionInfo.restcontext);
        if (result == '') {
            return '';
        }
        return result + '/';
...
function getCacheContext(connectionInfo) {
    let defaultValue = 'cache/';
    if (connectionInfo != null && connectionInfo.cachecontext != null) {
        let result = trimString(connectionInfo.cachecontext);
        if (result == '') {
            return defaultValue;
        }
        return result + '/';
 ...
 * If successful, will set the value of the $$SessionID$$ and $$SessionExpires$$ letiables accordingly.
    ...
    if (request != null) {
        ...
        if (pragmaStr != null) {
            ...
            if (sessIndx > -1) {
                ...
                let sessionId = pragmaStr.substr(sessIndx, (commaIndx - sessIndx));
                ...
                if (expiresIndx > -1) {
                    ...
                    let expiresMillis = parseInt(pragmaStr.substr(expiresIndx, (commaIndx - expiresIndx)));
...
exports.getSessionCookieId = getSessionCookieId;
...
function initSessionData(useSessionCookie, sessionCookiePrefix) {
    ...
    if ($$USE_SESSION_COOKIE$$ === true ) {
        $$SessionID$$ = getCookie(getSessionCookieId());
    }
}
exports.initSessionData = initSessionData;
...
function setCookie(keyName, value, expires) {
    if (keyName != null && value != null && keyName.length > 0 && value.length > 0) {
        let cookieString = keyName + '=' + escape(value);
        if (expires != null) {
...
function arrayIndexOf(arrayToSearch, toFind) {
    if (arrayToSearch != null) {
        let arrlength = arrayToSearch.length;
